...
-------------------
I recently added the ability to run more friendly queries using Named Arguments and C#4's `DynamicObject.TryInvokeMember` Method-on-the-fly syntax. Originally this was trying to be like Rails ActiveRecord (e.g. `var drinks = table.FindBy_CategoryID(8);`), but I figured "C# is NOT Ruby, and Named Arguments can be a lot more clear" (e.g. `var drinks = table.FindBy(CategoryID:8);`). In addition, Mark Rendle's Simple.Data is already supporting ActiveRecord style syntax, so ... why duplicate things?
